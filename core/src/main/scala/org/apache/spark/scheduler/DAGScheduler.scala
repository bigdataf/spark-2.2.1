...
class DAGScheduler(
  ...
  def submitJob[T, U](
    ...
    //类型为DAGSchedulerEventProcessLoop的对象eventProcessLoop，
    // 将任务提交JobSubmitted放置在event队列当中，eventThread后台线程将对该任务提交进行处理
    //该eventThread被定义在DAGSchedulerEventProcessLoop的父类EventLoop当中
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    try {
      ...
      //调用createResultStage创建Final Stage
...
private[scheduler] class DAGSchedulerEventProcessLoop(dagScheduler: DAGScheduler)
  ...
  private def doOnReceive(event: DAGSchedulerEvent): Unit = event match {
    ...
    //调用dagScheduler的handleJobSubmitted方法 完成整个任务的提交
