...
object SparkSubmit extends CommandLineUtils {
   ...
   * main class.
   * 使用所提供的参数提交应用程序，这两个步骤运行。首先，我们通过设置适当的类路径、系统属性和应用参数来准备基
   * 于集群管理器和部署模式运行子类的应用环境。其次，我们使用这个启动环境来调用子主体类的主要方法。
   */
  @tailrec
   ...
   * 使用所提供的启动环境运行子类的主方法。
请注意，如果运行群集部署模式或Python应用程序，则此主类将不是用户提供的类。
   */
  private def runMain(
    ...
    try {
      //复用反射加载childMainClass，也就是自己写的程序的主函数
